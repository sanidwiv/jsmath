...
function get_cur_div() {
    if (answer_step) {
        if (is_carry) {
            cur_div = '#answer-carry-' + (NUM_COLS - RIGHT_PADDING + carry_index - answer_array.length);
        } else {
            cur_div = '#answer-row-' + (NUM_COLS - RIGHT_PADDING + answer_index - answer_array.length);
        }
    } else {
        if (is_carry) {
            cur_div = '#carry-row-' + (NUM_COLS - RIGHT_PADDING + carry_index - num1_array.length - 1);
        } else {
            cur_div = '#step-' + steps_row + '-' + (NUM_COLS - RIGHT_PADDING + steps_index - steps_array[steps_row].length - steps_row);
        }
    }
    return (cur_div)
}
        ...
        if (is_carry) {
            ...
            if (steps_index == 0) {
                ...
                if (steps_row < n2) {
                    ...
                    answer_step = true;
                    carry_index = carry_answer.length - 2;
                }
                clear_carry();
...
function init() {
    // write number 1
    num1_start_index = NUM_COLS - RIGHT_PADDING - num1_array.length;
    for (var i = 0; i < num1_array.length; i++) {
        div_num = NUM_COLS - RIGHT_PADDING - num1_array.length + i;
        div = "#num1-row-" + div_num;
        $(div).html('<img class="img-fluid" src="img/normal/' + num1_array[i] + '.png">');
    }
    // write number 2
    num2_start_index = NUM_COLS - RIGHT_PADDING - num2_array.length;
    for (var i = 0; i < num2_array.length; i++) {
        div_num = NUM_COLS - RIGHT_PADDING - num2_array.length + i;
        div = "#num2-row-" + div_num;
        $(div).html('<img class="img-fluid" src="img/normal/' + num2_array[i] + '.png">');
    }

    mult_num = NUM_COLS - RIGHT_PADDING - num2_array.length - 1;
    ...
    for (var i = 0; i < num2_array.length; i++) {
        ...
        step_row = '#step-' + i;
        for (var j = 1; j <= NUM_COLS - RIGHT_PADDING; j++) {
